// Client-side localisation: German browsers get German copy, everyone
// else gets the English baked into the HTML. The dictionary maps CSS
// selectors to replacement HTML (arrays apply per matched element), so
// the markup itself stays single-source. A manual EN/DE toggle in the
// nav persists to localStorage and wins over browser language.

export const currentLang = (() => {
  try {
    const stored = localStorage.getItem('ocur-lang');
    if (stored === 'de' || stored === 'en') return stored;
  } catch {
    /* storage may be blocked — fall through to browser language */
  }
  const langs = navigator.languages || [navigator.language || 'en'];
  return langs.some((l) => /^de/i.test(l)) ? 'de' : 'en';
})();

// Strings used by main.js for content it renders dynamically.
...
const TITLE_DE = 'Ocur — Das KI-Betriebssystem für Unternehmen | Kostenlos starten';
const DESC_DE =
...
// EN | DE switch in the nav; the choice persists and wins over the
// browser language on the next visit.
export function initLangToggle() {
  ...
  ['en', 'de'].forEach((lang) => {
    ...
    btn.lang = lang;
    if (lang === currentLang) btn.classList.add('on');
