// Language plumbing for Design B. The German page is a real prerendered
// URL (/alt/de/, baked by scripts/prerender-de.mjs from
// src/alt/strings.de.js), so the URL is the source of truth for content
// language. This module: redirects German-browser visitors from /alt/ to
// /alt/de/ (honouring an explicit choice), renders the EN | DE nav toggle
// as real links, and applies the dictionary client-side as a dev fallback
// (?lang=de, where /alt/de/ doesn't exist yet). The language choice is
// shared with Design A via the same ocur-lang key/cookie.

import { TITLE_DE, DESC_DE, DE, DE_ATTRS } from './strings.de.js';

const query = new URLSearchParams(location.search).get('lang');
const pathLang = /^\/alt\/de(\/|$)/.test(location.pathname) ? 'de' : 'en';
export const currentLang = query === 'de' || query === 'en' ? query : pathLang;

const EN_PATH = '/alt/';
const DE_PATH = '/alt/de/';
...
// Explicit choice first, browser language second; an explicit /alt/de/
...
// Dev fallback: on the plain dev server /alt/de/ doesn't exist, so
